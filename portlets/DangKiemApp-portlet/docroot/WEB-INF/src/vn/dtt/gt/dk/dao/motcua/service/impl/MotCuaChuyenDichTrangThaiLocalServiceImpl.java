/**
 * Copyright (c) 2000-2013 Liferay, Inc. All rights reserved.
 *
 * This library is free software; you can redistribute it and/or modify it under
 * the terms of the GNU Lesser General Public License as published by the Free
 * Software Foundation; either version 2.1 of the License, or (at your option)
 * any later version.
 *
 * This library is distributed in the hope that it will be useful, but WITHOUT
 * ANY WARRANTY; without even the implied warranty of MERCHANTABILITY or FITNESS
 * FOR A PARTICULAR PURPOSE. See the GNU Lesser General Public License for more
 * details.
 */

package vn.dtt.gt.dk.dao.motcua.service.impl;

import java.util.List;

import com.liferay.portal.kernel.log.Log;
import com.liferay.portal.kernel.log.LogFactoryUtil;

import vn.dtt.gt.dk.dao.motcua.model.MotCuaChuyenDichTrangThai;
import vn.dtt.gt.dk.dao.motcua.service.base.MotCuaChuyenDichTrangThaiLocalServiceBaseImpl;

/**
 * The implementation of the mot cua chuyen dich trang thai local service.
 *
 * <p>
 * All custom service methods should be put in this class. Whenever methods are added, rerun ServiceBuilder to copy their definitions into the {@link vn.dtt.gt.dk.dao.motcua.service.MotCuaChuyenDichTrangThaiLocalService} interface.
 *
 * <p>
 * This is a local service. Methods of this service will not have security checks based on the propagated JAAS credentials because this service can only be accessed from within the same VM.
 * </p>
 *
 * @author win_64
 * @see vn.dtt.gt.dk.dao.motcua.service.base.MotCuaChuyenDichTrangThaiLocalServiceBaseImpl
 * @see vn.dtt.gt.dk.dao.motcua.service.MotCuaChuyenDichTrangThaiLocalServiceUtil
 */
public class MotCuaChuyenDichTrangThaiLocalServiceImpl
	extends MotCuaChuyenDichTrangThaiLocalServiceBaseImpl {
	/*
	 * NOTE FOR DEVELOPERS:
	 *
	 * Never reference this interface directly. Always use {@link vn.dtt.gt.dk.dao.motcua.service.MotCuaChuyenDichTrangThaiLocalServiceUtil} to access the mot cua chuyen dich trang thai local service.
	 */
	public java.util.List<vn.dtt.gt.dk.dao.motcua.model.MotCuaChuyenDichTrangThai> findByTrangThaiDichId(
			long trangThaiDichId){
		try{
			return motCuaChuyenDichTrangThaiPersistence.findByTrangThaiDichId(trangThaiDichId);
		}catch (Exception e) {
			_log.error(e);
		}
		return null;
	}
	
	public java.util.List<vn.dtt.gt.dk.dao.motcua.model.MotCuaChuyenDichTrangThai> findByDieuKienChuyenDich(
			java.lang.String dieuKienChuyenDich){
		try{
			return motCuaChuyenDichTrangThaiPersistence.findByDieuKienChuyenDich(dieuKienChuyenDich);
		}catch (Exception e) {
			_log.error(e);
		}
		return null;
	}
	
	public java.util.List<vn.dtt.gt.dk.dao.motcua.model.MotCuaChuyenDichTrangThai> findByTrangThaiNguonId(
			long trangThaiNguonId){
		try{
			return motCuaChuyenDichTrangThaiPersistence.findByTrangThaiNguonId(trangThaiNguonId);
		}catch (Exception e) {
			_log.error(e);
		}
		return null;
	}
	public java.util.List<vn.dtt.gt.dk.dao.motcua.model.MotCuaChuyenDichTrangThai> findByDieuKienChuyenDichAndTrangThaiNguonId(
			java.lang.String dieuKienChuyenDich, long trangThaiNguonId){
		try{
			return motCuaChuyenDichTrangThaiPersistence.findByDieuKienChuyenDichAndTrangThaiNguonId(dieuKienChuyenDich,trangThaiNguonId);
		}catch (Exception e) {
			_log.error(e);
		}
		return null;
	}
	
	public List<MotCuaChuyenDichTrangThai> findChuyenDichTrangThai(String dieuKienDichchuyen, int start, int end) {
		return motCuaChuyenDichTrangThaiFinder.findChuyenDichTrangThai(dieuKienDichchuyen, start, end);
	}

	public int countChuyenDichTrangThai(String dieuKienDichchuyen) {
		return motCuaChuyenDichTrangThaiFinder.countChuyenDichTrangThai(dieuKienDichchuyen);
	}
	
	private static Log _log = LogFactoryUtil.getLog(MotCuaChuyenDichTrangThaiLocalServiceImpl.class);
}