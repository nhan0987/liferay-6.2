...
<%
	...
	String noidungthaotac = "Mở xem Thụ lý hồ sơ thời điểm " + DateUtils.dateToString("dd/MM/yyyy HH:mm", new Date()) + " theo hoSoThuTucId " + idHoSoThuTuc; 
	String tenmanhinhdaidien = "Chi tiết Thụ lý hồ sơ";
	String importername = "";
...
tiepNhanHoSo.setParameter(Constants.TTHC_THUTUCHANHCHINH_ID, idthuTucHanhChinh);
tiepNhanHoSo.setParameter(Constants.THANH_PHAN_XU_LY, Constants.XU_LY_TLHS);
...
tiepNhan.setParameter(Constants.TTHC_THUTUCHANHCHINH_ID, idthuTucHanhChinh);
tiepNhan.setParameter(Constants.THANH_PHAN_XU_LY, Constants.XU_LY_TLHS);


%>

<!-- Tiếp nhận -->
<aui:form action="<%=tiepNhan.toString()%>" name="formTiepNhan2" id="formTiepNhan2" method="POST">
	<input type="text" hidden="true" name="<%=Constants.DIEU_KIEN_CHUYEN_DICH%>" value="<%=Constants.DIEU_KIEN_DICH_CHUYEN_PHU_HOP_QUY_DINH%>" />
</aui:form>
<aui:form action="<%=tiepNhanHoSo.toString()%>" name="formTiepNhan" id="formTiepNhan" method="POST">
	<input type="text" hidden="true" name="<%=Constants.DIEU_KIEN_CHUYEN_DICH%>" value="<%=Constants.DIEU_KIEN_DICH_CHUYEN_PHU_HOP_QUY_DINH%>" />
</aui:form>
...
<div id="divButton">
	<aui:button-row>
		...
		%>
		<button type="button" onclick="beforSummitForm('formTiepNhan2', '<%=tiepNhan.toString()%>')" class="btnThuLyHS form-control">Tiếp nhận</button>
		...
		%>
		<button type="button" onclick="beforSummitForm('formTiepNhan', '<%=tiepNhanHoSo.toString()%>')" class="btnThuLyHS form-control"><liferay-ui:message key="vn.gt.dk.qlhs.thulyhs.totiepnhan.qlhshschotiepnhan.tiepnhanvachuyentruongbophan"/></button>
		<button type="button" onclick="eventHideOrShowDiv('divYeuCauBoSung', 'true')" class=" btnThuLyHS form-control"><liferay-ui:message key="vn.gt.dk.qlhs.thulyhs.totiepnhan.qlhshschotiepnhan.yeucaubosung"/></button>
		<button type="button" onclick="eventHideOrShowDiv('divThongBaoViPham', 'true')" class="btnThuLyHS form-control" style="display: none;"><liferay-ui:message key="vn.gt.dk.qlhs.thulyhs.totiepnhan.qlhshschotiepnhan.thongbaovipham"/></button>
		<%
		}
		%>
...
<div id="divYeuCauBoSung">
	...
	<p style="text-align: left;"><span><liferay-ui:message key="vn.gt.dk.qlhs.thulyhs.totiepnhan.qlhshschotiepnhan.noidungyeucaubosung"/> <span style="color: red;">*</span> :</span></p>
	...
	<div class="CollapsiblePanel CollapsiblePanelOpen" id="CollapsiblePanel1" style="border-bottom: none">
		...
		<div class="row">
			<div class="col-xs-2 col-sm-2 col-md-2">
				<label class="textlabel"><liferay-ui:message key="vn.gt.dk.qlhs.thulyhs.totiepnhan.qlhshschotiepnhan.ngaythongbao"/></label>
			</div>
			<div class="col-xs-10 col-sm-10 col-md-10">
				<input type="text" style="width: 200px; float: left; cursor: pointer;" class="form-control-find-calendar" readonly="readonly"
					id="ngayThongBao" name="ngayThongBao"
					value="<%=DateUtils.getDateAfterNumberDay(1)%>"
					onclick="gtCalendar('ngayThongBao')"/>
				<span class="input-group-btn">
					<button class="btn_calendar btn-default " type="button" onclick="gtCalendar('ngayThongBao')"><i class="fa fa-calendar"></i></button>
				</span>
			</div>
		</div>
		...
		<div class="row">
			<div class="col-xs-2 col-sm-2 col-md-2">
				<label class="textlabel"><liferay-ui:message key="vn.gt.dk.qlhs.thulyhs.totiepnhan.qlhshschotiepnhan.ngayhoanthienhoso"/></label>
			</div>
			<div class="col-xs-10 col-sm-10 col-md-10">
				<input type="text" placeholder="Ngày hoàn thiện hồ sơ" class="form-control-find-calendar" style="width: 200px; float: left; cursor: pointer;" readonly="readonly"
...
thongBaoViPham.setParameter(ActionRequest.ACTION_NAME, "toTiepNhan_ThongBaoViPham");
thongBaoViPham.setParameter(Constants.ID_HO_SO_THU_TUC, idHoSoThuTuc);
thongBaoViPham.setParameter(Constants.TTHC_THUTUCHANHCHINH_ID, idthuTucHanhChinh);
thongBaoViPham.setParameter(Constants.THANH_PHAN_XU_LY, Constants.XU_LY_TLHS);
%>
<aui:form action="<%=thongBaoViPham.toString()%>" name="formThongBaoViPham" id="formThongBaoViPham" method="POST">
	<input type="text" hidden="true" name="<%=Constants.DIEU_KIEN_CHUYEN_DICH%>" value="<%=Constants.DIEU_KIEN_DICH_CHUYEN_VI_PHAM_ND187%>"/>
	<div id="divThongBaoViPham">
		<div><p style="text-align: left;"><span><liferay-ui:message key="vn.gt.dk.qlhs.thulyhs.totiepnhan.qlhshschotiepnhan.noidungkiennghi"/> <span style="color: red;">*</span> :</span></p></div>
		<div><textarea class="textAreaAutosize" id="noiDungKienNghi" name="noiDungKienNghi"><%=StringPool.BLANK%></textarea></div>
		<div>
			<p style="text-align: left; float: left; width: 15%"><span><liferay-ui:message key="vn.gt.dk.qlhs.thulyhs.totiepnhan.qlhshschotiepnhan.ngaythongbao"/></span></p>
			<input type="text" style="width: 200px; float: left; cursor: pointer;" class="form-control-find-calendar" readonly="readonly"
			id="ngayThongBaoViPham" name="ngayThongBaoViPham" onclick="gtCalendar('ngayThongBaoViPham')"/>
			<span class="input-group-btn">
				<button class="btn_calendar btn-default " type="button" onclick="gtCalendar('ngayThongBaoViPham')"><i class="fa fa-calendar"></i></button>
			</span>
		</div>
		<div>
			<aui:button-row>
				<button type="button" onclick="if(validatorThongBaoViPham()) beforSummitForm('formThongBaoViPham', '<%=thongBaoViPham.toString()%>')" class="btnThuLyHS form-control"><liferay-ui:message key="vn.gt.dk.qlhs.thulyhs.totiepnhan.qlhshschotiepnhan.thongbaovaochuyenbophannghiepvu"/></button>
				<button type="button" onclick="eventHideOrShowDiv('divThongBaoViPham', 'false')" class="btnThuLyHS form-control"><liferay-ui:message key="vn.gt.dk.qlhs.thulyhs.totiepnhan.qlhshschotiepnhan.quaylai"/></button>
			</aui:button-row>
		</div>
	</div>
</aui:form>
