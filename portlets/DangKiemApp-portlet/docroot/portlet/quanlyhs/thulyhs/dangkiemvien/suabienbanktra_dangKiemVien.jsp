...
<%@page import="vn.dtt.gt.dk.portlet.business.bienbankiemtra.BienBanKiemTraMessage"%>
...
<%! private static Log _log = LogFactoryUtil.getLog("portlet_quanlyhs_thulyhs_dangkiemvien_suabienbanktra_dangKiemVien_jsp"); %>
...
<%
	...
	_log.info("==idHoSoThuTuc==" + idHoSoThuTuc);
	_log.info("==idthuTucHanhChinh==" + idthuTucHanhChinh);
	_log.info("==idPhieuXyLyPhu==" + idPhieuXyLyPhu);
	_log.info("==confirmedInspectionId==" + confirmedInspectionId);
	_log.info("==idInspectionRecord==" + idInspectionRecord);
	_log.info("==idVehicleGroup==" + idVehicleGroup);
	_log.info("==idVehicleGroupInitial==" + idVehicleGroupInitial);
	...
	long inspectionRecordId = inspectionRecord.getId();
	...
	_log.info("==vehicleGroupInitialId==" + vehicleGroupInitial.getId());
	_log.info("==vehicleGroupId==" + vehicleGroup.getId());
	
	String vehicleClass = vehicleGroupInitial.getVehicleClass();
	String vehicleType = vehicleGroupInitial.getVehicleType();
	...
	if (inspectionRecord != null) {
		lstVehicleRecord = VehicleRecordLocalServiceUtil.findByInspectionRecordIdOrderbyAsc(inspectionRecordId);
...
<%
	...
	urlDuocPhanCongDangKiemVien.setParameter(Constants.BUTTON_TYPE, Constants.SUA_BIEN_BAN_KIEM_TRA);
	...
	urlLapBienBanKiemTra.setParameter(Constants.BUTTON_TYPE, Constants.LAP_BIEN_BAN_KIEM_TRA);
...
<liferay-ui:error key="<%= BienBanKiemTraMessage.ATTACH_FILE_MAX_SIZE %>" message="File đính kèm tải lên vượt quá dung lượng cho phép" />
<liferay-ui:error key="<%= BienBanKiemTraMessage.ERROR %>" message="Có lỗi xảy ra trong quá trình lưu dữ liệu" />
...
<div class="container" style="width: 100%; margin: 0px; padding: 0px;">
	...
	<div id="tab-1" class="tab-content current">
		...
		<%
		//_log.info("==VehicleClass==" + vehicleClass + "==idInspectionRecord==" + idInspectionRecord);
		
		List<InspectionCommonStatus> lstThongTinChung = InspectionCommonStatusLocalServiceUtil.findByVehicleClassAndTypeAndInspecRecordIdOderBySequenceNo(vehicleClass, 1, idInspectionRecord);
		if (Validator.isNull(lstThongTinChung)) { lstThongTinChung = new ArrayList<InspectionCommonStatus>(); }
		
		_log.info("==lstThongTinChung==" + lstThongTinChung.size());
					...
					<tr id="trtblThongTinChung<%=stt%>" <%=(itemTTC.getAllowEdit() == 1) ? (BienBanKiemTraBusiness.checkColorTrTable(itemTTC.getCommonStatus() + "")) : ""%>>
						...
						<td class="text-left paddingLeft05">
							...
							if (itemTTC.getCommonCode().equalsIgnoreCase("TTC_1_21") || itemTTC.getCommonCode().equalsIgnoreCase("TTC_1_22") ||
								...
								<input type="hidden" id="giaTriDangKyCodeTTC<%=stt%>" value="<%=giaTriKhaiBaoInitalCode%>" />
								<%
							} else {
								%>
		...
		<%
		List<InspectionRecordAttach> allInspectionRecordAttachs = InspectionRecordAttachLocalServiceUtil.findByInspectionRecordAttach(inspectionRecordId);
					...
					<div id="div_uploadDocOther_<%=wildcard%>">
						<br/>		
							...
							<div style="float: left; width: 48%; margin-right: 4%">
								...
								<input style="width: 97%;" class="validatorOtherFile" type="text" id="attachment_<%=wildcard%>" name="attachment_<%=wildcard%>" value="">
							</div>
...
<!-- template autofill -->			
	...
	<div id="tab-2" class="tab-content">
		...
		<%
		//_log.info("==VehicleClass====" + vehicleClass + "==idInspectionRecord==" + idInspectionRecord);
		List<InspectionCommonStatus> lstkiemTraChatLuongAnToanKt = InspectionCommonStatusLocalServiceUtil.findByVehicleClassAndTypeAndInspecRecordIdOderBySequenceNo(vehicleClass, 2, inspectionRecordId);
		if (Validator.isNull(lstkiemTraChatLuongAnToanKt)) { lstkiemTraChatLuongAnToanKt = new ArrayList<InspectionCommonStatus>(); }
		
		_log.info("==lstkiemTraChatLuongAnToanKt==" + lstkiemTraChatLuongAnToanKt.size());
		%>
		...
		<div style="padding: 1% 1% 1% 0;"><span style="font-weight: bold;">3. KIỂM TRA ĐỐI CHIẾU KẾT CẤU VÀ THÔNG SỐ KỸ THUẬT CỦA XE</span></div>
		<!-- table 2 -->
		...
		<%
			List<InspectionCommonStatus> lstKetCauKhiThais = InspectionCommonStatusLocalServiceUtil.findByVehicleClassAndTypeAndInspecRecordIdOderBySequenceNo(vehicleClass, 4, inspectionRecordId);
			if (Validator.isNull(lstKetCauKhiThais)) { lstKetCauKhiThais = new ArrayList<InspectionCommonStatus>(); }
		
			_log.info("==lstKetCauKhiThais==" + lstKetCauKhiThais.size() + "==idInspectionRecord==" + idInspectionRecord);
		...
		<%
		List<InspectionCommonStatus> lstXacNhanChiPhiPhatSinhs= InspectionCommonStatusLocalServiceUtil.findByVehicleClassAndTypeAndInspecRecordIdOderBySequenceNo(vehicleClass, 6, inspectionRecordId);
		if (Validator.isNull(lstXacNhanChiPhiPhatSinhs)) {
			lstXacNhanChiPhiPhatSinhs = new ArrayList<InspectionCommonStatus>(); 
		}
		_log.info("==lstXacNhanChiPhiPhatSinhs==" + lstXacNhanChiPhiPhatSinhs.size());
	...
	<aui:button-row cssClass="width100">
		<c:choose>
			<c:when test="<%= isView %>">
				<button type="button" class="btnThuLyHS form-control" style="width: 200px;" onclick='location.href="<%= HtmlUtil.escape(backURL) %>"' name="btnQuayLai">Quay lại</button>
			</c:when>
			<c:otherwise>
				<%
				...
				<portlet:actionURL var="dakiemtra_huyBienBanKiemTra" name="dakiemtra_huyBienBanKiemTra">
					...
					<portlet:param name="<%=Constants.ID_INSPECTION_RECORD%>" value="<%=String.valueOf(inspectionRecordId) %>"/>
				</portlet:actionURL>
	...
	<table width="100%">
		...
		<tr>
			<td colspan="2" align="center">
				<input type="button" class="" value="Đồng ý" id="btnXacNhanThongSoKt" onclick="javascript: func_xacnhanPheDuyet();">
				<input type="button" class="" value="Đồng ý" id="btnXacNhanThongTinChung" style="display: none;" onclick="javascript: func_xacnhanPheDuyetTTChung();">
				&nbsp;&nbsp; 
				<input type="button" class="" value="Hủy" onclick="javascript: func_huyPheDuyet();" >
			</td>
		</tr>
	</table>
</div>
...
<portlet:renderURL windowState="<%= LiferayWindowState.EXCLUSIVE.toString() %>" var="loadThongSoKyThuatURL">
	<portlet:param name="jspPage" value="/portlet/quanlyhs/thulyhs/dangkiemvien/suabienbanktra_loadThongSoKyThuat.jsp"/>
</portlet:renderURL>
...
var inputGiaTriThamChieu = $('#giaTriKiemTraTTC6');
...
/*
 *congThucBanhXeId
 isLoadPage: true neu luc moi dau load trang
 type: -1: la binh thuong(dung khi load trang), 0: la chua quyet dinh, 1: la dat, 2: la khong dat, 3: sua doi, 4: khong ap dung
 */
function loadThongSoKyThuat(congThucBanhXeId, isLoadPage, type) {
	...
	$.ajax({
		...
		data : {
			...
			<%= Constants.ID_VEHICLE_GROUP_INITIAL %>: <%= idVehicleGroupInitial %>,
			<%= Constants.ID_INSPECTION_RECORD %>: <%= inspectionRecordId %>,
		...
		success : function(data) {
			if (data != '{}') {
				...
				if (isThongTin == '1' && specificationcode == '4') {
					...
					var giaTriView = $('#giaTriKiemTraTTC' + stt).val();
				...
				if(commonCode === 'TTC_1_20') {
					...
					document.getElementById("listValueGiaTriNhap").style.height = "200px";
					giaTriTraVe = createTextArea(stt, giaTriKiemTra);
				} else {
