...
<%
System.out.println("/cauhinhmotcua/chuyendichtrangthai/chuyen_dich_trang_thai_edit.jsp");
	...
	long chuyenDichTrangThaiId = ParamUtil.getLong(request, "chuyenDichTrangThaiId");
    ...
    long trangThaiNguonId = ParamUtil.getLong(request, "trangThaiNguon");
    long trangThaiDichId = ParamUtil.getLong(request, "trangThaiDich");
    String dieuKienChuyenDich = ParamUtil.getString(request, "dieuKienChuyenDich");
    String ngayXoa = ParamUtil.getString(request, "ngayXoa");
    String nguoiXoa = ParamUtil.getString(request, "nguoiXoa");
    long soNgayXuLy = ParamUtil.getLong(request, Constants.NAM_NHAP);
    long phanNhomHoSoId = ParamUtil.getLong(request, Constants.NAM_NHAP);
    String hanhDongXuLy = ParamUtil.getString(request, Constants.NAM_NHAP);
    
	MotCuaChuyenDichTrangThai motCuaChuyenDichTrangThai = null;
	if(chuyenDichTrangThaiId > 0){
		motCuaChuyenDichTrangThai = MotCuaChuyenDichTrangThaiLocalServiceUtil.fetchMotCuaChuyenDichTrangThai(chuyenDichTrangThaiId);
		if(motCuaChuyenDichTrangThai != null){
		    trangThaiNguonId = motCuaChuyenDichTrangThai.getTrangThaiNguonId();
		    trangThaiDichId = motCuaChuyenDichTrangThai.getTrangThaiDichId();
		    dieuKienChuyenDich = motCuaChuyenDichTrangThai.getDieuKienChuyenDich();
		    soNgayXuLy = motCuaChuyenDichTrangThai.getSoNgayXuLy();
		    phanNhomHoSoId = motCuaChuyenDichTrangThai.getPhanNhomHoSoId();
		    hanhDongXuLy = motCuaChuyenDichTrangThai.getHanhDongXuLy();
		}
	}
	
	

    PortletURL actionUpdateChuyenDichTrangThai = renderResponse.createActionURL();
    actionUpdateChuyenDichTrangThai.setParameter(ActionRequest.ACTION_NAME, "updateChuyenDichTrangThai");
...
%>
<aui:form action="<%=actionUpdateChuyenDichTrangThai.toString()%>" name="myForm" id="myForm" method="POST">
	<div class="CollapsiblePanel CollapsiblePanelOpen" id="CollapsiblePanel1">
		...
		<div class="CollapsiblePanelContent" style="display: block;">
			...
			<div class="row">
				...
				<div class="col-xs-3 col-sm-3 col-md-3">
					<select name="trangThaiNguon" id="trangThaiNguon" size="1" class="form-control-find">
						...
						%>
						<option <%=(trangThaiHs.getId() == trangThaiNguonId) ? "selected" : ""%> value="<%=String.valueOf(trangThaiHs.getId())%>"><%=trangThaiHs.getTenTrangThai()%></option>
						<%}%>
					</select>
				</div>
				<div class="col-xs-2 col-sm-2 col-md-2">
				...
				<div class="col-xs-3 col-sm-3 col-md-3">
					<select name="trangThaiDich" id="trangThaiDich" size="1" class="form-control-find">
						...
						<%for (MotCuaTrangThaiHoSo trangThaiHs : allTrangThaiHoSo) {
						%>
						<option <%=(trangThaiHs.getId() == trangThaiDichId) ? "selected" : ""%> value="<%=String.valueOf(trangThaiHs.getId())%>"><%=trangThaiHs.getTenTrangThai()%></option>
						<%}%>
					</select>
				</div>
			</div>
			<div class="row">
				...
				<div class="col-xs-3 col-sm-3 col-md-3">
					<input type="text" placeholder="Nhập tên người xóa" class="form-control-find" name="nguoiXoa" id="nguoiXoa" value="<%=nguoiXoa%>" />
				</div>
				<div class="col-xs-2 col-sm-2 col-md-2">
				...
				<div class="col-xs-3 col-sm-3 col-md-3" style="margin-left: -35px;">
					<div class="input-group">
						<input type="text" placeholder="Từ ngày" name="ngayXoa" id="ngayXoa" value="<%=ngayXoa%>"
			...
			<div class="row">
				...
				<div class="col-xs-3 col-sm-3 col-md-3">
					<input type="text" placeholder="Nhập điều kiện dịch chuyển" class="form-control-find" name="dieuKienDichChuyen" id="dieuKienDichChuyen" value="<%=dieuKienChuyenDich%>" />
				</div>
				<div class="col-xs-2 col-sm-2 col-md-2">
					<label class="textlabel">Số ngày xử lý</label>
				</div>
				<div class="col-xs-3 col-sm-3 col-md-3">
					<input type="text" placeholder="Nhập số ngày xử lý" class="form-control-find" name="soNgayXuLy" id="soNgayXuLy" value="<%=soNgayXuLy%>" />
				</div>
			</div>
			<div class="row">
				<div class="col-xs-2 col-sm-2 col-md-2">
				...
				<div class="col-xs-10 col-sm-10 col-md-10">
					<input type="text" placeholder="Nhập hành động xử lý" class="form-control-find" name="hanhDongXuLy" id="hanhDongXuLy" value="<%=hanhDongXuLy%>" />
				</div>
			</div>
			<div class="row">
				<div class="col-xs-2 col-sm-2 col-md-2">
				...
				<div class="col-xs-3 col-sm-3 col-md-3">
					<select name="phanNhomHoSo" id="phanNhomHoSo" size="1" class="form-control-find">
						...
						%>
						<option <%=(phanNhomHs.getId() == phanNhomHoSoId) ? "selected" : ""%> value="<%=String.valueOf(phanNhomHs.getId())%>"><%=phanNhomHs.getTenPhanNhom()%></option>
						<%}%>
					</select>
				</div>
			</div>
			<input type="text" hidden="true" name="chuyenDichTrangThaiId" value="<%=chuyenDichTrangThaiId%>" />
			<div class="row">
				<div class="col-xs-2 col-sm-2 col-md-2">
