...
<%
	...
	AssessmentofImporterControl assessofImporter = AssessmentofImporterControlLocalServiceUtil.getAssessmentofImporterControl(assessmentOfImporterId);
	...
	addURL.setParameter(ActionRequest.ACTION_NAME, "updateTieuChiKiemSoatDoanhNghiep");
	addURL.setParameter(Constants.RETURN_URL, returnURL);
	
	// Ghi actionlog
	String noidungthaotac = "Vào chức năng Tạo mới Tiêu chí kiểm soát doanh nghiệp thời điểm "+ DateUtils.dateToString("dd/MM/yyyy HH:mm", new Date()); 
	String tenmanhinhdaidien = "Màn hình Tạo mới Tiêu chí kiểm soát doanh nghiệp";
	if (assessmentOfImporterId > 0) {
		noidungthaotac = "Vào chức năng Sửa Tiêu chí kiểm soát doanh nghiệp thời điểm "+ DateUtils.dateToString("dd/MM/yyyy HH:mm", new Date());
		tenmanhinhdaidien = "Màn hình Chỉnh sửa Tiêu chí kiểm soát doanh nghiệp";
	}
	String importername = "";
