...
<%
	...
	String evaluationperiod = assessmentofProduct.getEvaluationperiod();
	...
	addURL.setParameter(ActionRequest.ACTION_NAME, "updateTieuChiKiemSoatRuiRoSanPham");
	addURL.setParameter(Constants.RETURN_URL, returnURL);
	
	// Ghi actionlog
	String noidungthaotac = "Vào chức năng Tạo mới Tiêu chí kiểm soát rủi ro sản phẩm thời điểm "+ DateUtils.dateToString("dd/MM/yyyy HH:mm", new Date()); 
	String tenmanhinhdaidien = "Màn hình Tạo mới Tiêu chí kiểm soát rủi ro sản phẩm ";
	if (AssessmentofProductID > 0) {
		noidungthaotac = "Vào chức năng Sửa Tiêu chí kiểm soát rủi ro sản phẩm thời điểm "+ DateUtils.dateToString("dd/MM/yyyy HH:mm", new Date());
		tenmanhinhdaidien = "Màn hình Chỉnh sửa Tiêu chí kiểm soát rủi ro sản phẩm ";
	}
	String importername = "";
...
<div class="col-xs-12 col-sm-12 col-md-12" id="cot2">
	<div class="col-md-12" id="main">
		<div class="" style="padding-top: 1%;color: #777;text-align: left;font-size: 20px;padding-left: 10px;"><%=AssessmentofProductID == 0 ? "Tạo mới" : "Chỉnh sửa"%></div>
		<div class="col-xs-12 col-sm-12 col-md-12" id="cot2"><br>
			<div class="col-md-12" id="main">
				<aui:form action="<%=addURL%>" name="myForm" id="myForm" method="POST">
					<input type="hidden" name='<%= Constants.ID_ASSESSMENTOFPRODUCT %>' value="<%= AssessmentofProductID %>">
					...
					<div class="row">
						<div class="col-xs-2 col-sm-2 col-md-2">
							<label class="textlabel"> Mã sản phẩm</label>
						</div>
						<div class="col-xs-10 col-sm-10 col-md-10">
							<aui:input name="productcode" readonly ="true" value='<%=assessmentofProduct.getProductcode()%>'  
								maxlength="200" style="width: 400px" cssClass="form-control-find" type="text" size="30" label="" disabled="<%= true %>">
							</aui:input>
						</div>
					</div>
					...
					<div class="row">
						...
						<div class="col-xs-10 col-sm-10 col-md-10">
							<aui:input name="productname" readonly ="true" value='<%=assessmentofProduct.getProductname()%>'  
								maxlength="200" style="width: 400px" cssClass="form-control-find" type="text" size="30" label="" disabled="<%= true %>">
							</aui:input>
						</div>
					</div>
					<div class="row">
						<div class="col-xs-2 col-sm-2 col-md-2">
							<label class="textlabel"> Mã nhóm sản phẩm</label>
						</div>
						<div class="col-xs-10 col-sm-10 col-md-10">
							<aui:input name="productcodereference" readonly ="true" value='<%=assessmentofProduct.getProductcodereference()%>'  
								maxlength="200" style="width: 400px" cssClass="form-control-find" type="text" size="30" label="" disabled="<%= true %>">
							</aui:input>
						</div>
					</div>
					...
					<div class="row">
						<div class="col-xs-2 col-sm-2 col-md-2">
							<label class="textlabel"> Tên nhóm sản phẩm</label>
						</div>
						<div class="col-xs-10 col-sm-10 col-md-10">
							<aui:input name="productnamereference" readonly ="true" value='<%=assessmentofProduct.getProductnamereference()%>'  
								maxlength="200" style="width: 400px" cssClass="form-control-find" type="text" size="30" label="" disabled="<%= true %>">
							</aui:input>
						</div>
					</div>
					...
					<div class="AccordionPanelContent AssessmentOfImporterSpecificationConfig">
						<table class="table table-condensed table-hover table-bordered hh-table" id="<portlet:namespace/>dataTable"> 
							<thead>
								<tr>
									<th width="5%">STT</th>
									<th width="15%">Tiêu chí kiểm soát (Level0)</th>
									<th width="30%">Kết quả kiểm soát (Level1)</th>
							...
							<tbody>
								<% 
									...
									DmDataItem itemLevel3 = null;
									DmDataItem itemLevel0 = DmDataItemLocalServiceUtil.findDataItemByDataGroupIdAndItemCode(GetterUtil.getLong(specificationgroupcode), level0.getSpecificationcode());
									if ( (itemLevel0 != null && itemLevel0.getCode2().equalsIgnoreCase("1"))
									|| (itemLevel0 != null && itemLevel0.getCode2().equalsIgnoreCase("2"))
									|| (itemLevel0 != null && itemLevel0.getCode2().equalsIgnoreCase("3"))
									|| (itemLevel0 != null && itemLevel0.getCode2().equalsIgnoreCase("4"))
									|| (itemLevel0 != null && itemLevel0.getCode2().equalsIgnoreCase("5")) 
									|| (itemLevel0 != null && itemLevel0.getCode2().equalsIgnoreCase("6")) ){
										itemLevel3 = DmDataItemLocalServiceUtil.findDataItemByDataGroupIdAndItemCode(itemLevel0.getDataGroupId(), itemLevel0.getCode1());										 
									}
									%>
									
									<%
									if (itemLevel3 != null && itemLevel3.getLevel()==3) {
									%>
									<tr>
									<td colspan="5" class="text-left "><label class="aicon"><i class="mar_r10 xanh fa fa-circle" style="font-size: 13x;"></i><%=itemLevel3.getDescription()%> ". " <%=itemLevel3.getName()%></label><br></td>
									<td class="text-right"><span id=""></span></td>
									</tr>																		
									<%
									} 
									%>
