...
<%
	...
	long AssessmentofInspectorID = ParamUtil.getLong(request, Constants.ID_ASSESSMENTOFINSPECTOR);
	...
	String evaluationperiod = assessmentofInspector.getEvaluationperiod();
	...
	addURL.setParameter(ActionRequest.ACTION_NAME, "updateTieuChiDanhGiaNhanSu");
	addURL.setParameter(Constants.RETURN_URL, returnURL);
	
	// Ghi actionlog
	String noidungthaotac = "Vào chức năng Tạo mới Tiêu chí đánh giá nhân viên thời điểm "+ DateUtils.dateToString("dd/MM/yyyy HH:mm", new Date()); 
	String tenmanhinhdaidien = "Màn hình Tạo mới Tiêu chí đánh giá nhân viên";
	if (AssessmentofInspectorID > 0) {
		noidungthaotac = "Vào chức năng Sửa Tiêu chí đánh giá nhân viên thời điểm "+ DateUtils.dateToString("dd/MM/yyyy HH:mm", new Date());
		tenmanhinhdaidien = "Màn hình Chỉnh sửa Tiêu chí đánh giá nhân viên";
	}
	String importername = "";
...
<div class="col-xs-12 col-sm-12 col-md-12" id="cot2">
	<div class="col-md-12" id="main">
		<div class="" style="padding-top: 1%;color: #777;text-align: left;font-size: 20px;padding-left: 10px;"><%=AssessmentofInspectorID == 0 ? "Tạo mới" : "Chỉnh sửa"%></div>
		<div class="col-xs-12 col-sm-12 col-md-12" id="cot2"><br>
			<div class="col-md-12" id="main">
				<aui:form action="<%=addURL%>" name="myForm" id="myForm" method="POST">
					<input type="hidden" name='<%= Constants.ID_ASSESSMENTOFINSPECTOR %>' value="<%= AssessmentofInspectorID %>">
					...
					<div class="row">
						<div class="col-xs-2 col-sm-2 col-md-2">
							<label class="textlabel"> Mã đăng kiểm viên</label>
						</div>
						<div class="col-xs-10 col-sm-10 col-md-10">
							<aui:input name="Inspectorcontactcode" value='<%=assessmentofInspector.getInspectorcontactcode()%>'  
								maxlength="200" style="width: 400px" cssClass="form-control-find" type="text" size="30" label="" disabled="<%= true %>">
							</aui:input>
						</div>
					</div>
					...
					<div class="row">
						<div class="col-xs-2 col-sm-2 col-md-2">
							<label class="textlabel"> Tên đăng kiểm viên</label>
						</div>
						<div class="col-xs-10 col-sm-10 col-md-10">
							<aui:input name="Inspectorname" value='<%=assessmentofInspector.getInspectorname()%>'  
								maxlength="200" style="width: 400px" cssClass="form-control-find" type="text" size="30" label="" disabled="<%= true %>">
							</aui:input>
						</div>
					</div>
					<div class="row">
						<div class="col-xs-2 col-sm-2 col-md-2">
							<label class="textlabel"> Vị trí</label>
						</div>
						<div class="col-xs-10 col-sm-10 col-md-10">
							<aui:input name="Inspectorposition" value='<%=assessmentofInspector.getInspectorposition()%>'  
								maxlength="200" style="width: 400px" cssClass="form-control-find" type="text" size="30" label="" disabled="<%= true %>">
							</aui:input>
						</div>
					</div>
					...
					<div class="AccordionPanelContent AssessmentOfImporterSpecificationConfig">
						<table class="table table-condensed table-hover table-bordered hh-table" id="<portlet:namespace/>dataTable"> 
							<thead>
								<tr>
									<th width="5%">STT</th>
									<th width="15%">Tiêu chí đánh giá NV(Level0)</th>
