...
public class DmDataItemFinderImpl extends BasePersistenceImpl<DmDataItem> implements DmDataItemFinder {
	...
	public List<DmDataItem> findByArrayCode0(String arrayCode0) {
		...
		try {
			session = openSession();
			StringBuilder query = new StringBuilder();
			query.append("SELECT * FROM dm_dataitem where code_0 in (" + arrayCode0 + ")");
			
			SQLQuery q = session.createSQLQuery(query.toString());
			q.setCacheable(false);
			q.addEntity("DmDataItem", DmDataItemImpl.class);
			
			log.info("=========findByArrayCode0===" + query.toString() );
			QueryPos qPos = QueryPos.getInstance(q);
	...
	public List<DmDataItem> findBySpecificationCode(String specificationcode, String vehicleClass) {
		...
		try {
			session = openSession();
			StringBuilder query = new StringBuilder();
			query.append("SELECT * FROM dm_dataitem where ( altername='"+vehicleClass.trim()+"' or altername is null or altername='' ) and datagroupid in ");
	...
	public List<DmDataItem> findByVehicleClassAndDatagroupid(long datagroupid, String vehicleClass) {
		...
		try {
			session = openSession();
			StringBuilder query = new StringBuilder();
			query.append("SELECT * FROM dm_dataitem where ( altername like '"+vehicleClass.trim()+"%' or altername is null or altername='' ) and datagroupid= "+datagroupid);
	...
	public List<DmDataItem> findByVehicleClassAndDatagroupidByLoaiPhuongTien(long datagroupid, String vehicleClass) {
		...
		try {
			session = openSession();
			StringBuilder query = new StringBuilder();
			query.append("SELECT * FROM dm_dataitem where ( altername like '%"+vehicleClass.trim()+"%' and level > 1 ) and datagroupid= "+datagroupid);
	...
	public List<DmDataItem> findByDataGroupCodeNameAndDataItemAlterName(String groupName, String alterName) {
		...
		try {
			session = openSession();
			StringBuilder query = new StringBuilder();
			query.append("SELECT * FROM dm_dataitem d where altername = ? and datagroupid in (SELECT code FROM dm_datagroup where name = ?) order by CONVERT(name USING utf8) COLLATE utf8_polish_ci");
	...
	public List<DmDataItem> findByNameGroupAndMultilAlterName(String groupName, String alterName1, String alterName2) {
		...
		try {
			session = openSession();
			StringBuilder query = new StringBuilder();
			query.append("SELECT * FROM dm_dataitem d where altername in ('" + alterName1+"', '" + alterName2 + "') and datagroupid in (SELECT code FROM dm_datagroup where name = ?) order by CONVERT(name USING utf8) COLLATE utf8_polish_ci");
	...
	public List<DmDataItem> findDmDataItemByDataGroupIdAndLevel(long groupId, int level) {
		...
		try {
			session = openSession();
			StringBuilder query = new StringBuilder();
			query.append("SELECT * FROM dm_dataitem d where datagroupid = ? and Level = ? order by CONVERT(name USING utf8) COLLATE utf8_polish_ci");
	...
	public List<DmDataItem> findByDataGroupCodeNameAndDataItemCode0(String groupName, String code_0) {
		...
		try {
			session = openSession();
			StringBuilder query = new StringBuilder();
			query.append("SELECT * FROM dm_dataitem d where code_0 = ? and datagroupid in (SELECT code FROM dm_datagroup where name = ?) order by CONVERT(name USING utf8) COLLATE utf8_polish_ci");
