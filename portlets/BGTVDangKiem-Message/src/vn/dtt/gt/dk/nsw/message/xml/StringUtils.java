...
public class StringUtils {
	...
	*/
	public final static boolean isEmpty(final String tobetested) {
		if ((tobetested != null) && (!tobetested.trim().equals(""))) { return false; }
		return true;
	}
	
	public final static String getString(final Object tobetested, final String emptyString) {
		if (tobetested != null) { return tobetested.toString().intern(); }
		return emptyString;
	}
	
	public final static String getString(final String tobetested, final String emptyString) {
		if ((tobetested != null) && (!tobetested.trim().equals(""))) { return tobetested.intern(); }
		return emptyString;
	}
	...
	*/
	public static String[] concatStringArray(String[] sourceArray, String[] array2Add) {
		if (sourceArray == null) { return array2Add; }
		
		if (array2Add == null) { return sourceArray; }
	...
	*/
	...
	public static final String formatMessage(Object[] values, int[] lens, String delimiter) {
		...
		for (int i = 0; i < len - 1; i++) {
			buffer.append(format(values[i], lens[i])).append(delimiter);
		}
	...
	public static void testFormat() {
		...
		for (int i = total; i >= 0; i--) {
			...
			buffer.append(formatStr("[" + count++ + "] LEFT", 12, StringUtils.ALIGN_LEFT, ' ')).append(
				formatStr(testStr, 11, StringUtils.ALIGN_LEFT, fill)).append("\n").append(
				formatStr("[" + count++ + "] LEFT ", 12, StringUtils.ALIGN_LEFT, ' ')).append(
				formatStr(testStr, 20, StringUtils.ALIGN_LEFT, fill)).append("\n").append(
				formatStr("[" + count++ + "] LEFT", 12, StringUtils.ALIGN_LEFT, ' ')).append(
				formatStr(testStr, 6, StringUtils.ALIGN_LEFT, fill)).append("\n").append(
				formatStr("[" + count++ + "] RIGHT", 12, StringUtils.ALIGN_LEFT, ' ')).append(
				formatStr(testStr, 11, StringUtils.ALIGN_RIGHT, fill)).append("\n").append(
				formatStr("[" + count++ + "] RIGHT", 12, StringUtils.ALIGN_LEFT, ' ')).append(
				formatStr(testStr, 20, StringUtils.ALIGN_RIGHT, fill)).append("\n").append(
				formatStr("[" + count++ + "] RIGHT", 12, StringUtils.ALIGN_LEFT, ' ')).append(
				formatStr(testStr, 6, StringUtils.ALIGN_RIGHT, fill)).append("\n");
		...
		for (int i = total; i >= 0; i--) {
			...
			buffer.append(format("[" + count++ + "] LEFT", 12, StringUtils.ALIGN_LEFT, ' ')).append(
				format(testStr, 11, StringUtils.ALIGN_LEFT, fill)).append("\n").append(
				format("[" + count++ + "] LEFT ", 12, StringUtils.ALIGN_LEFT, ' ')).append(
				format(testStr, 20, StringUtils.ALIGN_LEFT, fill)).append("\n").append(
				format("[" + count++ + "] LEFT", 12, StringUtils.ALIGN_LEFT, ' ')).append(
				format(testStr, 6, StringUtils.ALIGN_LEFT, fill)).append("\n").append(
				format("[" + count++ + "] RIGHT", 12, StringUtils.ALIGN_LEFT, ' ')).append(
				format(testStr, 11, StringUtils.ALIGN_RIGHT, fill)).append("\n").append(
				format("[" + count++ + "] RIGHT", 12, StringUtils.ALIGN_LEFT, ' ')).append(
				format(testStr, 20, StringUtils.ALIGN_RIGHT, fill)).append("\n").append(
				format("[" + count++ + "] RIGHT", 12, StringUtils.ALIGN_LEFT, ' ')).append(
				format(testStr, 6, StringUtils.ALIGN_RIGHT, fill)).append("\n");
		...
		Object[] values = new Object[] {
			"A", new Integer(20), "", "abcd efg hijklmn", new Boolean(true), new Double(2.345)
		};
		int[] lens = new int[] {
			1, 4, 20, 10, 6, 10
		};
		...
		if (strTarget != null) {
			...
			while (strCheck.length() != 0) {
				...
				if (begin == -1) {
					...
					if (begin != 0 || (begin == 0 && bContainNull)) vc.add(index++, (begin == 0) ? null : strCheck.substring(0, begin));
