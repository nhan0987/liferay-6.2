 ...
 * @date: 3 Jul 2009
 */
public class DKConfigurationManager {
	private static Log _log = LogFactoryUtil.getLog(DKConfigurationManager.class);
	public static final String fileNameAll = File.separator + "dk.properties";
	private static final DKConfigurationManager instance = new DKConfigurationManager();
	...
	private DKConfigurationManager() {
	}
	
	public static DKConfigurationManager getInstance() {
		return instance;
	}
	...
	public ConcurrentProperties<String, Object> getConcurrentProperties(String fileName) {
		if (!concurrentProperties.containsKey(fileName)) {
			_log.info("========LOAD FILE CAU HINH NAY==============" + fileName);
	...
	private Properties loadProperties(String module1) {
		...
		try {
			InputStream pin = DKConfigurationManager.class.getResourceAsStream(module1);
			props.load(pin);
			return props;
	...
	public static long getLongProp(String key, long defaultVal) {
		try {
			...
			if (result != null && result.toString().trim().length() > 0) { 
				return ConvertUtil.convertToLong(result.toString()); 
			}
		} catch (Exception e) {
			_log.error(e);
		}
