...
public class SendMessgaeFunctions {
	...
	public static String insertMesssageToQueueAPI(String userName,
			TthcHoSoThuTuc hoSoThuTuc, String version, String messageId,
			String messageFunction, int messageType, String xmlData,
			String xmlSignature, int level, long phieuXuLyPhuId) 
			throws SystemException {
		
		String messageIdSentToNSW = messageId;
		
		Header header = MessageReturnUtils.crateHeader(messageFunction,
			messageType, userName, messageIdSentToNSW, version, hoSoThuTuc);

		xmlData = XuLyMessage.createContentSendFromBgtvtToNSW(header, xmlData, messageIdSentToNSW, xmlSignature);
		
		SendMessgaeFunctions.insertMesssageToQueueAPI(header, xmlData,
			MessageKey.HTTH_BGT_TO_NSW,
			TrangThaiHoSo.DA_BO_SUNG_CHO_TIEP_NHAP, messageIdSentToNSW,
			level, hoSoThuTuc.getId(), phieuXuLyPhuId);
	
		return xmlData;
	}
	
	private static long insertMesssageToQueueAPI(Header header, String data,
			String direction, int requestState, String uuid, int level,
			long hoSoThuTucId, long phieuXuLyPhuId) throws SystemException {
